...
class Product {
  ...
  constructor(name, price) {
    ...
    try {
      ...
      throw new Error("Product can't be created", { case: e });
    }
  }
}
```
...
Javascript introduced WeakRef and FinalizationRegistry to provide more control over
garbage collection and manage memory usage more efficiently
...
these feature used for VR , AI and dose run in browser that need heavy memory. if not release
memory you will get problem.

The FinalizationRegistry is like event lister for garbage collector.

```js
let obj = { message: "Hello, world!" };

const registry = new FinalizationRegistry((heldValue) => {
  console.log(`Object with held value ${heldValue} has`);
});
...
Provide a way to parse template literals with a function, allowing for custom
processing of template string

```js
function tag(string, ...values) {
  console.log("Strings length:", string.length);
  console.log("Values length:", values.length);

  return string[0] + value.join("");
}
const value = "world";
...
and this another example of $

```js
function format(strings, ...value) {
  let output = "";
...
a flag to match any character including line breaks
...
ensures that the match must start at the current position
...
allow you to assign a name to capture 
matched data more readable and accessible using <>.

```js 
regex = /(?<year>\d{4})-(?<month>\d{2})-(?<day>\d{2})/;
result = regex.exec("2023-06-01");
console.log("Named groups", result.group.year, result.group.month,
  result.groups.day
)
```

### Unicode property escapes flag

allow matching Unicode character 

```js 
regex = /\p{Script=Greek}/u;
console.log("Greek char", regex.test('a'));

regex = /\p{P}/gu;
console.log("Punctuation char", regex.test("Hello, World! @ 1234"))
```